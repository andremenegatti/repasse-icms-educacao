...
# Mudando variavel indicativa de nota imputada
df_municipios <- df_municipios %>% 
  mutate(`Nota IDEB Imputada` = ifelse(nota_imputada, 'Sim', NA))
...
sp.mapa$`Variacao (%)` <- sp.mapa[["var_perc_transf_secretaria"]] * 100

title <- 'V.A. (75%), Educ. (18,5%), RCL (6%), Area Inundada (0,5%)'
...
mapa_variacao <- tm_shape(sp.mapa) + 
  ...
  tm_dots("Nota IDEB Imputada", size = 0.1, col = "green", shapeNA = NA, title = "Nota IDEB Imputada", id = 'municipio')

tmap_save(mapa_variacao, '2019_08_14_simulacao_secretaria/mapa_var_perc_IMP.html')
