...
for (i in 1:13) {
  sheet <- readxl::read_excel('Dados/dados_DIPAM_2005_2017.xlsx', sheet = i) %>%
    # Lembrando: a distribuicao em t obedece os indices de participacao calculados para t-2
...
historico_icms_anual <- readxl::read_excel('Dados/historico_arrecadacao_icms_sp.xlsx')  %>% 
...
df_municipios <- historico_icms_anual %>% 
  ...
  right_join(df_municipios, by = 'ano_distribuicao') %>% # Joining variable: ano_distribuicao <<<<<<
  select(ano_distribuicao, ano_calculo, codigo:ind_participacao, total_icms_estado)


#### CALCULANDO VARIAVEIS IMPORTANTES ####
...
# Para o valor adicionado, considera-se a participacao do municipio no total estadual NOS DOIS EXERCICIOS ANTERIORES AO DA APURACAO
# Vamos criar variaveis com lags do valor adicionado para calcular essa participacao
df_municipios <- df_municipios %>%
  arrange(ano_calculo, municipio) %>% 
  nest(-municipio) %>% 
  mutate(data = map(.x = data,
                    .f = ~ .x %>% mutate(lag_valor_adicionado = lag(valor_adicionado),
                                         lag2_valor_adicionado = lag(lag_valor_adicionado)))) %>% 
  unnest() %>% 
  mutate(valor_adicionado_2anos = valor_adicionado + lag_valor_adicionado,
         valor_adicionado_2anos_2 = lag_valor_adicionado + lag2_valor_adicionado)
...
#### VERIFICANDO SE CONSEGUIMOS CALCULAR O INDICE DE PARTICIPACAO A PARTIR DOS DADOS DAS VARIAVEIS ####

# Calculando indice de participacao a partir dos dados
df_municipios <- df_municipios %>% 
  mutate(ind_participacao_calculado = 76 * share_valor_adicionado 
         + 13 * share_pop
         ...
         + 2 * 1/645,
         ind_participacao_calculado_2 = ind_participacao_calculado - 76 * share_valor_adicionado + 76 * share_valor_adicionado_2anos,
         ind_participacao_calculado_3 = ind_participacao_calculado - 76 * share_valor_adicionado + 76 * share_valor_adicionado_2anos_2)
...
ggplot(data = df_municipios %>% select(ano_calculo, starts_with('dif_perc_')) %>% gather(-ano_calculo, key = 'var', value = 'value')) +
  geom_histogram(mapping = aes(x = value, group = var, fill = var), position = 'identity', alpha = 0.3) +
...
map_dbl(.x = list(ind_participacao_calculado, ind_participacao_calculado_2, ind_participacao_calculado_3),
        .f = ~ rmse(ind_participacao, .x)) %>% set_names(c('share_valor_adicionado', 'share_valor_adicionado_2anos', 'share_valor_adicionado_2anos2'))
...
# Diferencas percentuais absolutas para os resultados obtidos com share_valor_adicionado_2anos parecem suficientemente proximos de zero,
# com media de 0,11% e maximo de 2,74%:
summary(abs(dif_perc_dipam_calculado_2))

# Considerando apenas 2018, a diferenca percentual absoluta máxima é 0,7%
df_municipios %>% filter(ano_distribuicao == '2018') %>% select(dif_perc_dipam_calculado_2) %>% mutate(dif_perc_dipam_calculado_2 = abs(dif_perc_dipam_calculado_2)) %>% summary()
...
df_municipios %>% 
  ...
  saveRDS('2019_08_14_simulacao_secretaria/df_municipios_etapa1.rds')
