...
df_municipios <- readRDS('2019_08_14_simulacao_secretaria/df_municipios_etapa1.rds') %>% 
...
df_municipios <- df_municipios %>% 
  ...
  mutate(media_ideb_ano = mean(nota_ideb_imp, na.rm = TRUE),
         mediana_ideb_ano = median(nota_ideb_imp, na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(nota_ideb_imp = if_else(is.na(nota_ideb_imp), media_ideb_ano, nota_ideb_imp),
         nota_ideb_imp_mediana = if_else(is.na(nota_ideb_imp), mediana_ideb_ano, nota_ideb_imp))
...
df_municipios %>% 
  select(-media_ideb_ano,
         -nota_ideb_imp,
         -nota_ideb_anterior) %>% 
  saveRDS('2019_08_14_simulacao_secretaria/df_municipios_etapa2.rds')
