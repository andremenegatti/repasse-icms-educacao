...
df1 <- df_municipios %>% 
  select(municipio,
         var_transf25_nota:var_transf10_evolucao_e_nota_X_matriculas_municipal) %>% 
...
df2 <- df_municipios %>% 
  select(municipio,
         ...
         nota_imputada,
         var_perc_transf25_nota:var_perc_transf10_evolucao_e_nota_X_matriculas_municipal
  ) %>% 
  gather(-municipio, -pop, -fracao_pop_abaixo_15, -receitas_correntes_2018, -nota_imputada, key = 'simulacao', value = 'var_transf_perc_cota2018') %>% 
...
df_variacoes <- df_variacoes %>% 
  ...
  select(municipio, simulacao, nota_imputada, var_transf_total, var_transf_perc_cota2018, var_transf_perc_rc2018, var_transf_por_hab_abaixo_15anos, pop, receitas_correntes_2018)
...
maiores_variacoes_valor_absoluto %>% 
  mutate_all(as.character) %>% 
  mutate_all(str_replace, pattern = '\\.', replacement = ',') %>% 
  salvar_tabela('Resultados2/maiores_variacoes_valor_absoluto.csv')
